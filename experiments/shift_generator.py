...
def rsna_gender_and_prev_shift(
    test_df,
    ...
    gender_shifted = rsna_gender_shift(
        test_df,
        target_female_proportion,
        target_dataset_size=None,
        random_state=random_state,
    )
    return rsna_prev_shift(
        gender_shifted, target_prevalence, target_dataset_size, random_state
    )


def rsna_gender_shift(
    ...
    female_test = test_df.loc[test_df["Patient Gender"] == "F"]
    male_test = test_df.loc[test_df["Patient Gender"] == "M"]
    ...
    min_sizes = np.round(np.array([len(female_test), len(male_test)]))
...
def rsna_subpopulation_shift(
    test_df, target_abnormal_neg, target_dataset_size=None, random_state=None
):
    if isinstance(target_abnormal_neg, Tuple):
        target_abnormal_neg = float(
            np.random.uniform(target_abnormal_neg[0], target_abnormal_neg[1], 1)
        )
    test_df_positives = test_df.loc[test_df.label_rsna_pneumonia]
    test_df_negatives = test_df.loc[~test_df.label_rsna_pneumonia]
    test_df_normal = test_df.loc[test_df["class"] == "Normal"]
    test_df_abnormal_neg = test_df.loc[
        test_df["class"] == "No Lung Opacity / Not Normal"
    ]

    n_positives = len(test_df_positives)
    n_negatives = len(test_df_negatives)

    n_neg_abnormal = target_abnormal_neg * n_negatives
    n_neg_normal = (1 - target_abnormal_neg) * n_negatives

    target_size = np.array([n_positives, n_neg_abnormal, n_neg_normal])
    min_sizes = np.round(
        np.array([n_positives, len(test_df_abnormal_neg), len(test_df_normal)])
    )
    with np.errstate(divide="ignore", invalid="ignore"):
        factor = target_size / min_sizes
    min_factor = np.max(factor[~np.isinf(factor)])
    ...
    target_size = target_size.astype(int)
    sub_positives = test_df_positives.sample(
        axis=0, replace=False, n=target_size[0], random_state=random_state
    )
    sub_normal = test_df_normal.sample(
        axis=0, replace=False, n=target_size[2], random_state=random_state
    )
    sub_abnormal = test_df_abnormal_neg.sample(
        axis=0, replace=False, n=target_size[1], random_state=random_state
    )
    subpopulation_shift = pd.concat([sub_positives, sub_normal, sub_abnormal])
    return subpopulation_shift


def rsna_prev_shift(
    test_df, target_prevalence, target_dataset_size=None, random_state=None
):
    positives = test_df.loc[test_df.label_rsna_pneumonia]
    negatives = test_df.loc[~test_df.label_rsna_pneumonia]

    n_positives = len(positives)
    if isinstance(target_prevalence, Tuple):
        target_prevalence = float(
            np.random.uniform(target_prevalence[0], target_prevalence[1], 1)
        )
    n_negative_high_prev = (1 - target_prevalence) * (n_positives / target_prevalence)

    target_size = np.round(np.array([n_positives, n_negative_high_prev]))
    min_sizes = np.array([n_positives, len(negatives)])
    with np.errstate(divide="ignore", invalid="ignore"):
        factor = target_size / min_sizes
    min_factor = np.max(factor[~np.isinf(factor)])
    print(factor, min_factor)
    ...
    sub_neg = negatives.sample(
        axis=0, replace=False, n=target_size[1], random_state=random_state
    )
    sub_positive = positives.sample(
        axis=0, replace=False, n=target_size[0], random_state=random_state
    )
    high_prev_shift = pd.concat([sub_positive, sub_neg])
    return high_prev_shift
